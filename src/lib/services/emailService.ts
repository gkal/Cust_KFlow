/**
 * ⚠️ SERVER-SIDE ONLY EMAIL SERVICE ⚠️
 * =================================
 * 
 * WARNING: This file contains Node.js-specific code and CANNOT be used in the browser!
 * DO NOT import this file in any client-side code or components!
 * 
 * If you need to send emails from client-side code, use emailApiService.ts instead,
 * which provides a browser-safe alternative.
 * 
 * For a production application, you would need:
 * 1. A server API endpoint that imports this file and sends emails
 * 2. Client-side code that makes HTTP requests to that endpoint
 */
...
export const sendFormSubmissionNotification = async (data: FormSubmissionData) => {
  try {
    ...
    const smtpResult = await smtpTransporter.sendMail(mailOptions);
    logDebug('Email sent successfully via SMTP', smtpResult);
    return smtpResult;
  } catch (error) {
...
export const send404Notification = async (data: NotFound404NotificationData) => {
  try {
    ...
    const smtpResult = await smtpTransporter.sendMail(mailOptions);
    logDebug('404 notification email sent successfully via SMTP', smtpResult);
    return smtpResult;
  } catch (error) {
